...
function saveProgress() { localStorage.setItem('dateGame_v2', JSON.stringify(state.progress)); }
function loadProgress() {
  try { const d = JSON.parse(localStorage.getItem('dateGame_v2')); if (d) state.progress = d; } catch(e) {}
...
function startNarrative() {
  ...
  document.getElementById('narText').innerHTML = nar.intro;
...
class GameEngine {
  ...
  start() { this.resize(); this.running = true; this.lastTime = performance.now(); this.updateHUD(); this.loop(performance.now()); }
  ...
  render() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      ctx.strokeStyle = i%2===0?'#ec4899':'#a855f7'; ctx.lineWidth=.5; ctx.stroke();
    }
    ctx.globalAlpha = 1;
    ...
    this.items.forEach(item => {
      ...
      // 外发光 (radius ×1.3 = r*2.86, alpha ×1.3)
  ...
  updateHUD() {
    ...
    te.textContent=Math.ceil(this.time);
...
function showResult(score) {
  ...
  let msg = `<div style="font-style:italic;margin-bottom:12px;color:var(--purple);">"${endingText}"</div>`;

  if (starsEarned === 0) msg += '这次约会似乎不太顺利……<br>再试一次吧！';
  else if (starsEarned === 1) msg += '感受到了一点心动。<br>继续努力吧！';
  else if (starsEarned === 2) msg += '不错的约会体验！<br>心动值在上升~';
  else msg += '完美约会！他的心，你已经抓住了。';

  if (gotCardStar) {
    msg += `<br><br><span style="color:var(--pink-deep);font-weight:700;">✦ ${ch.name}卡牌 +1★（${prog.cardStars}/3）✦</span>`;
  } else if (starsEarned < 3) {
    msg += `<br><br><span style="font-size:.8em;color:var(--text-light);">获得三星可点亮卡牌星（当前 ${prog.cardStars}/3 ★）</span>`;
  }

  if (justUnlocked) {
    msg += `<br><span style="color:var(--pink-deep);font-weight:700;">🎉 解锁了${ch.name}的卡牌！</span>`;
  }

  document.getElementById('resultMsg').innerHTML = msg;
  showScreen('screen-result');

  if (justUnlocked) setTimeout(() => showUnlock(ch), 2500);
}
